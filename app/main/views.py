...
def followed_by(username):
    ...
    follows = [{'user':item.followed,'timestamp':item.timestamp}
               for item in pagination.items]
    return render_template('followers.html',title='Followed by',
                           user=user,pagination=pagination,follows=follows,
...
@login_required
def test():
    followeds = current_user.followed.all()
    users = [User.query.get(f.followed_id) for f in followeds]
    #return "%s" %followeds[0]
